...
Para escalar privilegios a administrador explotamos el servicio `CloudMe` usando un [**exploit**](https://www.exploit-db.com/exploits/44470) de buffer overflow.

Como siguiente paso, `Chisel` nos ayudara para hacer `portforwarding`, y acceder al servicio CloudMe en nuestra propia máquina.
Generamos con msfvenom el `shellcode`.

Remplazamos el shellcode del exploit con el nuestro, ejecutamos y finalmente obtenemos nuestra shell como `admin`.
